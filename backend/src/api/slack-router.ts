...
/**
 * Database update data structure
 */
 ...
 */
function resolveRealChannelId(channelId: string): string {
  ...
  if (realChannelId) {
    ...
    if (channelId && channelId.startsWith('C') && channelId.length > 8) {
      logger.warn(
        `Unknown channel ID ${channelId}, falling back to configured channel ${realChannelId}`
      );
      return realChannelId;
    }
  }
 ...
 */
router.use((req: SlackRequest, res: Response, next: NextFunction): void => {
  ...
  if (process.env['NODE_ENV'] === 'production' && process.env['SLACK_SIGNING_SECRET']) {
    ...
    if (parseInt(timestamp) < fiveMinutesAgo) {
      ...
      res.status(401).json({ error: 'Unauthorized - Request too old' });
      return;
    }
...
router.post('/events', async (req: Request, res: Response): Promise<void> => {
  try {
    const eventRequest = req.body as SlackEventRequest;

    // Handle Slack URL verification challenge
    if (eventRequest.type === 'url_verification') {
      logger.info('Slack URL verification challenge received');
      res.json({ challenge: eventRequest.challenge });
      return;
    }
 ...
 */
async function processRevisionFeedback(event: SlackEvent, feedback: string): Promise<void> {
  try {
    ...
    try {
      let supabase = await serviceManager.getService<SupabaseClient>('supabase');
      let retryCount = 0;
      const maxRetries = 2;
      ...
      while (retryCount <= maxRetries) {
        try {
          if (!supabase) {
            ...
            supabase = await serviceManager.getService<SupabaseClient>('supabase');
            if (!supabase) {
          ...
          logger.warn(`Database lookup attempt ${retryCount} failed, retrying:`, errorMessage);
    ...
    let threadMessages: { messages?: SlackMessage[] } | null = null;
    try {
      logger.info(
        `Attempting to fetch thread messages from channel ${realChannelId} with ts ${event.thread_ts}`
      );
      ...
      const errorMessage = threadsError instanceof Error ? threadsError.message : 'Unknown error';
      logger.error('Failed to fetch thread messages, will use fallback approach:', errorMessage);
      ...
      try {
        ...
        if (recentMessages.messages) {
          ...
          logger.info(
            `Fallback successful: found ${threadMessages.messages?.length || 0} related messages`
          );
        }
      } catch (fallbackError) {
        const errorMessage = fallbackError instanceof Error ? fallbackError.message : 'Unknown error';
        logger.error('Fallback approach also failed:', errorMessage);
    ...
    if (threadMessages?.messages?.length && threadMessages.messages.length > 0) {
      ...
      for (const message of threadMessages.messages) {
        ...
        const bookingMatch =
          message.text?.match(/booking[_\s\-]*([a-zA-Z0-9_]+)/i) ||
          ...
          elements: [
            {
              ...
              text: { type: 'plain_text' as const, text: '✅ Approve Revised Email' },
            ...
            {
              type: 'button' as const,
              text: { type: 'plain_text' as const, text: '📝 Revise Again' },
...
router.post('/interactions', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    try {
      payload = JSON.parse(req.body.payload) as SlackPayload;
    } catch (parseError) {
      const errorMessage = parseError instanceof Error ? parseError.message : 'Unknown error';
      logger.error('Failed to parse payload JSON:', errorMessage, { payload: req.body.payload });
    ...
    if (!payload.type) {
      ...
      res.status(400).json({ error: 'Invalid payload: missing type' });
      return;
    }
    ...
    if (payload.type === 'block_actions' && payload.actions && payload.actions[0]) {
      ...
      // Get services
      let slack = await serviceManager.getService<WebClient>('slack');
      let supabase = await serviceManager.getService<SupabaseClient>('supabase');
      ...
      const realChannelId = resolveRealChannelId(
        payload.channel?.id || getServiceConfig('slack').channelId
      );
      ...
      switch (action_id) {
        ...
        case 'revise_email':
          ...
          logger.info(
            `Stored revision context: booking=${bookingId}, thread_ts=${updateData.thread_ts}, channel=${updateData.channel_id}`
          );
          break;
        ...
        while (!dbUpdateSuccess && retryCount < maxRetries) {
          try {
            if (!supabase) {
              ...
              const freshSupabase = await serviceManager.getService<SupabaseClient>('supabase');
              if (freshSupabase) {
            ...
            if (error) {
              ...
              if (retryCount === maxRetries - 1) {
                ...
                logger.error(
                  `All database update attempts failed for booking ${bookingId}. Action will be processed without database update.`
                );
            ...
            if (retryCount < maxRetries) {
              ...
              logger.error(
                `All database update attempts failed for booking ${bookingId}. Button action processed but not persisted.`
              );
      ...
      const followUpChannelId = resolveRealChannelId(
        payload.channel?.id || getServiceConfig('slack').channelId
      );
...
router.get('/health', (_req: Request, res: Response): void => {
  res.json({
    ...
    endpoints: ['/api/slack/interactions', '/api/slack/events', '/api/slack/health'],
